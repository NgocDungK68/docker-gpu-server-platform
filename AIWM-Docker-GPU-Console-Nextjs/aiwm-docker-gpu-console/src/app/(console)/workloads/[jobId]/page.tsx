...
export default function WorkloadDetailPage() {
  ...
  return <><Link href="/workloads" className="mb-4 inline-flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-slate-950"><ArrowLeft className="size-4" />Danh sách workloads</Link><PageHeader eyebrow="Managed workload" title={job.name} description={job.id} actions={<><Badge value={job.status} />{stoppable.includes(job.status) && <Button variant="danger" onClick={() => setStopOpen(true)}><Square className="size-4" />{job.status === "QUEUED" ? "Huỷ job" : "Dừng job"}</Button>}</>} />
    <div className="grid gap-5 xl:grid-cols-[1.25fr_.75fr]">
      <div className="space-y-5"><Card><CardHeader title="Trạng thái điều phối" description="Desired state từ Control Plane và actual state do Agent xác nhận" /><div className="p-5"><div className="flex flex-wrap items-center gap-3"><Badge value={job.status} /><p className="text-sm font-medium text-slate-700">{job.statusReason || "Không có mô tả trạng thái"}</p></div><div className="mt-5 grid gap-3 sm:grid-cols-3"><InfoTile icon={<Cpu className="size-5" />} label="GPU request" value={`${job.resources.gpuCount} × ${job.resources.gpuModel || "bất kỳ"}`} note={`VRAM ≥ ${formatMiB(job.resources.minVramMiB ?? 0)}`} /><InfoTile icon={<Server className="size-5" />} label="Server" value={job.assignment ? shortId(job.assignment.serverId, 18) : "Chưa xếp"} note={job.assignment ? formatDateTime(job.assignment.assignedAt) : "Đang ở hàng đợi"} /><InfoTile icon={<Clock3 className="size-5" />} label="Priority / strategy" value={`${job.priority} · ${job.strategy}`} note="Priority cao xếp trước; FIFO khi bằng nhau" /></div></div></Card>
    ...
    </div><Dialog open={stopOpen} onClose={() => setStopOpen(false)} onConfirm={stop} busy={stopMutation.isPending} destructive title="Dừng workload này?" description="Control Plane sẽ xếp lệnh STOP_CONTAINER cho Agent sở hữu. Container legacy trên cùng server không bị ảnh hưởng." confirmLabel="Dừng workload" /></>;
}
