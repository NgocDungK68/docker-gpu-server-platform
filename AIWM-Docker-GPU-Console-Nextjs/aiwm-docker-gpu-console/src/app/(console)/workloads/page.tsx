...
import { profileLabel } from "@/lib/utils/display";
...
import { formatDateTime, relativeTime, shortId } from "@/lib/utils/format";

const filters: Array<{ value: "ALL" | JobStatus; label: string }> = [{ value: "ALL", label: "Mọi trạng thái" }, { value: "ASSIGNED", label: "Đã lên lịch" }, { value: "RUNNING", label: "Đang chạy" }, { value: "QUEUED", label: "Đang chờ" }, { value: "STARTING", label: "Đang khởi động" }, { value: "FAILED", label: "Thất bại" }, { value: "STOPPED", label: "Đã dừng" }];
...
export default function WorkloadsPage() {
  ...
  return <><PageHeader title="Workloads" actions={<Link href="/workloads/new" className="btn btn-primary"><Plus className="size-4" />Tạo workload</Link>} />
    <Card><div className="flex flex-col gap-3 border-b border-slate-100 p-4 md:flex-row"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><input className="field-input pl-9" placeholder="Tìm tên workload, ID hoặc image…" value={search} onChange={(event) => setSearch(event.target.value)} /></div><select className="field-select md:w-56" value={status} onChange={(event) => setStatus(event.target.value as "ALL" | JobStatus)}>{filters.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></div>
      {query.isLoading ? <TableSkeleton rows={7} /> : query.error ? <div className="p-5"><ErrorState message={query.error.message} onRetry={query.refetch} /></div> : jobs.length === 0 ? <EmptyState icon={<Boxes className="size-6" />} title="Chưa có workload" description="Tạo workload hoặc thay đổi bộ lọc hiện tại." action={<Link href="/workloads/new" className="btn btn-primary">Tạo workload</Link>} /> : <div className="overflow-x-auto"><table className="data-table"><thead><tr><th>Workload</th><th>Trạng thái</th><th>Yêu cầu</th><th>Tính cần thiết</th><th>Lịch sử dụng</th><th>Máy chủ</th><th>Cập nhật</th><th /></tr></thead><tbody>{jobs.map((job) => <tr key={job.id}><td><Link href={`/workloads/${job.id}`} className="font-bold text-slate-950 hover:text-[var(--brand-red-dark)]">{job.name}</Link></td><td><Badge value={job.status} label={job.status === "ASSIGNED" ? "Đã lên lịch · Chưa chạy" : undefined} /></td><td><p className="font-semibold">{job.resources.gpuCount} × {profileLabel(job.resources.performanceProfile)}</p><p className="mt-0.5 text-xs text-slate-400">VRAM tối thiểu {(job.resources.minVramMiB ?? 0) / 1024} GB/GPU</p></td><td><span className="rounded-lg bg-slate-100 px-2.5 py-1 font-bold text-slate-700">{job.necessityLabel}</span></td><td><p>{formatDateTime(job.requestedStartAt)}</p><p className="text-xs text-slate-500">→ {formatDateTime(job.requestedEndAt)}</p><p className="text-xs text-slate-500">{(job.ttlSeconds ?? 0) / 60} phút</p></td><td>{job.assignment ? <><p className="font-mono text-xs text-slate-700">{shortId(job.assignment.serverId, 16)}</p><p className="mt-0.5 text-xs text-slate-400">{job.assignment.gpuUuids.length} GPU</p></> : <span className="text-slate-400">Chưa xếp</span>}</td><td>{relativeTime(job.updatedAt)}</td><td><Link href={`/workloads/${job.id}`} aria-label={`Xem ${job.name}`} className="grid size-8 place-items-center rounded-lg text-slate-400 hover:bg-red-50 hover:text-[var(--brand-red)]"><ChevronRight className="size-4" /></Link></td></tr>)}</tbody></table></div>}
    </Card>
  </>;
}
