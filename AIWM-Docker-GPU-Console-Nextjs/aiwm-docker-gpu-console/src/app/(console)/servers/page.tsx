...
export default function ServersPage() {
  ...
  return <><PageHeader eyebrow="Agent inventory" title="Máy chủ Docker GPU" description="GPU sẵn sàng chỉ được tính trên server có heartbeat và inventory mới. Drain chặn placement mới." />
    <Card><div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 p-4">
      <div className="relative w-full max-w-md"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" />
        <input aria-label="Tìm máy chủ" className="field-input pl-9" placeholder="Tên, machine ID, địa chỉ, nhãn…" value={search} onChange={(event) => setSearch(event.target.value)} /></div>
      ...
      {query.isLoading ? <TableSkeleton /> : query.error ? <div className="p-5"><ErrorState message={query.error.message} onRetry={query.refetch} /></div> :
        servers.length === 0 ? <EmptyState icon={<ServerIcon className="size-6" />} title="Không tìm thấy máy chủ" description="Đổi từ khoá hoặc khởi chạy Agent để tạo inventory." /> :
        <div className="overflow-x-auto"><table className="data-table"><thead><tr><th>Máy chủ / địa chỉ</th><th>Agent / scheduling</th><th>GPU sẵn sàng</th><th>Containers</th><th>Heartbeat / inventory</th><th>Nhãn</th></tr></thead>
          <tbody>{servers.map((server) => {
            ...
            return <tr key={server.id}>
              ...
              <td><Badge value={server.status} /><p className="mt-1 max-w-56 text-xs text-slate-500">{server.schedulingReason || "Sẵn sàng nhận job"}</p></td>
              <td><b>{free}/{server.gpus.length}</b><p className="text-xs text-slate-500">{[...new Set(server.gpus.map((gpu) => gpu.model))].join(", ")}</p></td>
              <td><p>{active.filter((container) => container.origin === "MANAGED").length} Managed</p><p>{active.filter((container) => container.origin !== "MANAGED").length} External</p></td>
