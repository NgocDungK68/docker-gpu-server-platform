...
const strategies = [
  ...
  ["Bin Packing", "Score = 10 × GPU trống còn lại − GPU đang bận."],
...
export default function SchedulerPage() {
  ...
  return <><PageHeader title="Scheduler" description="Quyết định cấp phát từ Control Plane. Chọn strategy khi tạo job."
    actions={<Button onClick={run} disabled={mutation.isPending}><Play className="size-4" />Chạy một chu kỳ</Button>} />
    ...
    <Card className="mt-5"><CardHeader title="Quyết định gần đây" description="Kết quả placement thực của các job đã được gán GPU" />
      {jobs.isLoading ? <TableSkeleton rows={4} /> : jobs.error ? <ErrorState message={jobs.error.message} /> : assigned.length === 0 ? <EmptyState icon={<Cpu />} title="Chưa có placement" description="Tạo một workload để xem server, GPU và điểm số được chọn." /> :
        <div className="overflow-x-auto"><table className="data-table"><thead><tr><th>Job</th><th>Strategy</th><th>Score</th><th>Reservation</th><th>Giải thích</th></tr></thead>
          <tbody>{assigned.slice(0, 20).map((job) => <tr key={job.id}><td>{job.name}</td><td>{job.assignment?.strategy}</td><td>{job.assignment?.score}</td><td><Badge value={job.assignment?.reservationState ?? ""} /></td><td className="max-w-lg">{job.assignment?.reason}</td></tr>)}</tbody>
        </table></div>}
    </Card>
  </>;
}
