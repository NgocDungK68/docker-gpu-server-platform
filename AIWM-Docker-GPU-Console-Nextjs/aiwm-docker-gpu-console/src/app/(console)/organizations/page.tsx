...
export default function OrganizationsPage() {
  ...
  return <div className="space-y-6">
    <div><h1 className="text-2xl font-bold text-slate-900">Đơn vị và tài khoản</h1><p className="mt-2 text-sm text-slate-500">Quản lý quyền truy cập nội bộ và ownership của logical resource pool.</p></div>
    {message && <p role="status" className="rounded-xl bg-emerald-50 p-3 text-sm text-emerald-800">{message}</p>}
    {(organizations.isError || users.isError) && <p role="alert" className="rounded-xl bg-red-50 p-3 text-red-700">Không tải được metadata. <button onClick={() => { void organizations.refetch(); void users.refetch(); }}>Thử lại</button></p>}
    ...
    <section className="grid gap-5 xl:grid-cols-[1.4fr_1fr]">
      ...
      <form className="surface-card space-y-4 p-5" onSubmit={e => { e.preventDefault(); if (userId && !window.confirm("Lưu thay đổi và thu hồi các phiên đăng nhập cũ của tài khoản?")) return; saveUser.mutate(); }}>
        ...
        <label className="block"><span className="field-label">Quyền</span><select className="field-select" value={account.role} onChange={e => setAccount({...account,role:e.target.value as UserInput["role"]})}><option value="ORGANIZATION_USER">ORGANIZATION_USER</option><option value="ADMIN">ADMIN</option></select></label>
        <div><p className="field-label">Đơn vị</p>{userId ? <p className="rounded-lg bg-slate-50 p-3 text-sm">{organizations.data?.find(o => o.id===account.organizationId)?.name} · Ownership tài khoản không đổi.</p> : <OrganizationSelect organizations={(organizations.data ?? []).filter(o => o.enabled)} value={account.organizationId} onChange={id => setAccount({...account,organizationId:id})} />}</div>
