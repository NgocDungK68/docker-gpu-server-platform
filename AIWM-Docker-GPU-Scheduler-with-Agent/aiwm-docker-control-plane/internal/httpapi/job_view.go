...
func publicJob(j domain.Job) JobView {
	...
	for i:=range events { events[i].Reason=publicReason(events[i].Reason) }
	j.Events=events
	environment := make(map[string]string, len(j.Environment))
	for key := range j.Environment {
		environment[key] = "[redacted]"
	}
	return JobView{OrganizationID:j.OrganizationID, NecessityLabel: policy.NecessityLabel(j.NecessityLevel), AllocationIntent: j.AllocationIntent, Policy: j.Policy, ID: j.ID, Name: j.Name, Image: j.Image, Backend: j.Backend, Command: j.Command, Environment: environment,
...
func publicReason(reason string) string {
	if strings.Contains(reason,", score ") { return "Đã chọn server trong đơn vị và reserve GPU; chờ Agent xác nhận runtime." }
	return reason
}
